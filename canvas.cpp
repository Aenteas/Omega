    ...
    prevWhite{NULL}
{
    ...
    create_hexagons();
    connect(this, SIGNAL (updateGameState(unsigned int)), parent, SLOT (updateGameState(unsigned int)));
}
...
vector<Point> Canvas::hexa_centers() const
{
    ...
    unsigned int colCount=boardSize;
    for(unsigned int row=1; row<=2*boardSize-1; ++row)
    {
...
void Canvas::create_hexagons()
{
    ...
    unsigned int colCount = boardSize;
    unsigned int idx = 0;
    for(unsigned int row=0; row<2*boardSize-1; ++row)
    {
        hexagons.push_back({});
...
Hexagon* Canvas::locate_hexagon(const QPoint& pos){
    ...
    */
    ...
    if(y >= (2*boardSize-1)*h_row+radius*(1-cos(M_PI/6))-eps) return NULL;
    // if we are below the table
    if(y <= eps) return NULL;
    ...
    if(x<=radius*(1-cos(M_PI/6))+eps) return NULL;
    // if we are to the right of the table
    if(x>=2*radius*((2*boardSize-1)-abs(boardSize-row))-radius*(1-cos(M_PI/6))-eps) return NULL;

    unsigned int col = ceil(x/(2*radius));
...
void Canvas::mousePressEvent(QMouseEvent* ev)
{
    if(active){ // if game has started
        ...
        if(hexagon and hexagon->updateColor(currentColor)){
            ...
            // schedule paintEvent
            repaint();
            emit updateGameState(hexagon->idx);
        }
    }
}

void Canvas::aiBotMovedEvent(unsigned int whiteIdx, unsigned int blackIdx){
